...
class DashboardController extends Controller
{
    ...
    public function addEmployee()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            } elseif (strlen(trim($data['position'])) < 3) {
                $error[] = "Position must be at least 3 characters long.";
            } elseif (preg_match('/[^a-zA-Z0-9\s]/', $data['position'])) {
                $error[] = "Position should not contain special characters.";
            }
            if (empty($data['department'])) {
                $error[] = "Department is required.";
            } elseif (!in_array($data['department'], ['HR', 'Sales', 'IT'])) {
            ...
            if (empty($data['date_of_joining'])) {
                ...
                $date = \DateTime::createFromFormat('Y-m-d', $data['date_of_joining']);
    ...
    public function addsalary()
    {
        ...
        $error = [];
        $dashboardModel = $this->model('AdminDashboard');
        $employeesModel = $this->model('Employees');
        if (isset($_POST['submit'])) {
            ...
            if (empty($employee_id) || empty($basic_salary) || empty($hra) || empty($deductions)) {
                ...
                $employeeDetails = $employeesModel->getById("employees", $employee_id);
                if (!$employeeDetails) {
                    $error[] = "User doesn't exist";
                } else if ($dashboardModel->checkSalaryExists($employee_id)) {
